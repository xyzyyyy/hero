...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
  try {
    ...
    let randomkamu = `${Math.floor(Math.random() * 71)}`.trim() //hehe Biar Susah Menang :v
    let Aku = (randomaku * 1)
    ...
    if (new Date - global.db.data.users[m.sender].lastbansos > 300000) {
      if (Aku > Kamu) {
        ...
        m.reply( m.chat, `Sorry Gan Lu g Berhasil Korupsi bansos Dan Tidak masuk penjara karna kamu *melarikan diri🏃*`, m)
        global.db.data.users[m.sender].lastbansos = new Date * 1
      }
    } else m.reply(m.chat, `Kamu sudah Melakukan Korupsi Bansos 💰\nDan kamu harus menunggu selama agar bisa korupsi bansos kembali \n▸ 🕓 ${timers}`, m)
  } catch (e) {
    throw `Kok erorr`
  }
}
